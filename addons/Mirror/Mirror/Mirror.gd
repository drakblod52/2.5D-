...
##
@export var OverrideTexture: Texture2D

@export var z_far: float = 100
...
func _process(delta):
	...
	mirror.get_active_material(0).set_shader_parameter("override_tex", OverrideTexture)
	...
	if near < z_far:
		#valid frustum
		cam.set_frustum(mirror.mesh.size.x, frustum_offset, near, z_far)
